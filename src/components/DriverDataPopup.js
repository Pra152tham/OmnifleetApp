...
const DriverDataPopup = ({ visible, onClose, category, driverStatusDetails, index }) => {
  const [driversStatusData, setDriversStatusData] = useState([]);
  ...
  const exportToText = async () => {
    ...
    const path = `${RNFS.DocumentDirectoryPath}/DriverData.txt`;
  ...
  const exportToExcel = async () => {
    try {
      ...
      const path = `${RNFS.DocumentDirectoryPath}/DriverData.xlsx`;
  ...
  }, [userId, startDate, category, visible]);
  ...
  const filterDriverData = () => {
    let filteredStatusDetails = [];
    if (category === 'top') {
      filteredStatusDetails = driverStatusDetails.topDrivers[index] ? [driverStatusDetails.topDrivers[index]] : [];
    } else if (category === 'bottom') {
      filteredStatusDetails = driverStatusDetails.bottomDrivers[index] ? [driverStatusDetails.bottomDrivers[index]] : [];
    } else {
      filteredStatusDetails = driverStatusDetails.allDrivers ? [driverStatusDetails.allDrivers] : [];
    }
    setDriversStatusData(filteredStatusDetails);
    setModalData(filteredStatusDetails);
  };
...
export default DriverDataPopup;

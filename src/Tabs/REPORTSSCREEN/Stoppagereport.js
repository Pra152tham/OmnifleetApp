...
const Stoppagereport = ({ navigation }) => {
    ...
    const fetchReportsData = async () => {
        try {
            ...
            console.log(group1);
            const response = await axios.get(`https://www.novusomnifleet.com/hitech-api/report/getStoppageReport/${formattedStartDate}/${formattedEndDate}/${userId}/null`);
    ...
    const copyAllToClipboard = () => {
        const textContent = filteredData.map(item => 
            `VehicleReg No: ${item.vehRegNo}
            Driver Name: ${item.driverName}
            Device ID: ${item.deviceId}
            Start Date: ${starttime(item)}
            End Date: ${endttime(item)}
            Stoppage (in hours): ${item.duration}
            Location: ${item.stop_address}`
        ).join('\n\n');
    ...
    const starttime =(item)=> {
        ...
        // Split the string by the underscore and select the first three parts (year, month, day)
        const dateParts = timestamp.split('_').slice(0, 3);

        // Join the parts with a hyphen to form the date
        const date  = dateParts.join('-');
        return date;
    }
    const endttime =(item)=> {
        const timestamp = item.end_time;
    ...
    return (
        <View style={styles.container}>
            <Text style= {styles.heading}>Stoppage Report</Text>
                            ...
                            <TouchableOpacity onPress={() => toggleExpand(index)}>
                                <View style={styles.cardHeader}>
                                    <View style={styles.subheader}>
                                        <Text style={styles.cardHeaderText}>VehicleReg No:</Text>
                                        <Text style={styles.cardHeaderText}>{item.vehRegNo}</Text>
                                    </View>
                            ...
                            {expandedIndex === index && (
                                <View style={styles.cardContent}>
                                    ...
                                    <View style={styles.row}>
                                        ...
                                        <Text style={styles.cell}>{endttime(item)}</Text>
                                    </View>
                                    <View style={styles.row}>
                                        <Text style={styles.headerCell}>Stoppage (in hours):</Text>
                                        <Text style={styles.cell}>{item.duration}</Text>
                                    </View>
                                    <View style={styles.row}>
                                        <Text style={styles.headerCell}>Location:</Text>
                                        <Text style={styles.celladdress}>{item.stop_address}</Text>
                                    </View>
                                </View>
                            )}
                        </View>
